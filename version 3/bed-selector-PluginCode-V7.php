...
function sondercare_handle_submission()
{
    ...
    $data = json_decode(stripslashes($_POST["data"]), true);
     ...
     // ✅ NEW: Get phone number from quotes table
    ...
    if ($result) {
        ...
        wp_send_json_error(["message" => "Failed to save quote"]);
    }
}
...
function sondercare_save_quote()
{
    ...
    // Insert into quotes table (NOW WITH EMAIL, PHONE, POSTAL CODE)
    $result = $wpdb->insert(
        $table_name,
        [
            "quote_number" => sanitize_text_field($data["quoteNumber"]),
        ...
        ["%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%s", "%f", "%s", "%s", "%d"]
    );
 
    if ($result) {
        // Trigger Google Sheets sync (will update in Phase 2)
        sondercare_sync_to_google_sheets($wpdb->insert_id);
        wp_send_json_success(["message" => "Quote saved successfully"]);
    } else {
        wp_send_json_error(["message" => "Failed to save quote"]);
    }
...
{
    if (!isset($answers['q5'])) return 'None';
    ...
    return $mattressMap[$answers['q5']] ?? 'N/A';
}
...
function sondercare_get_accessories_list($answers)
{
    ...
    // Functionality accessories (q8)
    if (isset($answers['q8']) && is_array($answers['q8'])) {
        foreach ($answers['q8'] as $item) {
            $accessories[] = ucwords(str_replace('-', ' ', $item));
        }
    }
    
    // Bedding (q9)
    if (isset($answers['q9']) && is_array($answers['q9'])) {
        foreach ($answers['q9'] as $item) {
            $accessories[] = ucwords(str_replace('-', ' ', $item));
        }
    }
...
{
    if (!isset($answers['q10'])) return 'N/A';
    ...
    return $deliveryMap[$answers['q10']] ?? 'N/A';
}

function sondercare_get_warranty($answers)
{
    if (!isset($answers['q11'])) return 'N/A';
    
    return $answers['q11'] === '5year-parts-and-labor' ? '5 Year Parts & Labor' : '5 Year Parts Only';
}

// 7. SEND EMAIL NOTIFICATION WITH HTML
function sondercare_send_notification_email($data)
{
    $to = "info@sondercare.com, kyle.sobko@sondercare.com";
